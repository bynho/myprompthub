...
import { Tag, Calendar, Folder, Copy, Trash, Edit } from 'lucide-react';
...
const SavedPromptCard: React.FC<SavedPromptCardProps> = ({ prompt }) => {
  ...
  const handleCopy = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    copyToClipboard(prompt.generatedContent);
  ...
  return (
    <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200">
      <div className="p-5">
        ...
        <p className="text-gray-600 mb-4 line-clamp-3">{prompt.generatedContent}</p>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          <div className="flex items-center text-sm text-gray-500">
            ...
            <span>{new Date(prompt.savedAt).toLocaleDateString()}</span>
          </div>
        </div>
